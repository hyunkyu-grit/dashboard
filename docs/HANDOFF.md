...
### 대사 스택: 드래그 팬 + 전 테너 열 복원 (2026-08-12)

[OWNER — "백테스트랑 시뮬레이션에서 좌우로 드래그하는 부분을 만들어서
잘리는 부분도 볼 수 있게 해주고, 물리적으로 잘린 테너들도 복원해줘"].
2026-08-10 의 폭 규율("좌우 스크롤 하기 싫음" → KRD 전 기간 0 테너 열
숨김)을 뒤집는 룰링이다 — 대사 표가 트레이딩 시스템처럼 전 테너를 다
보여주되, 넘치는 폭은 끌어서 본다.
...
- **가로 드래그 팬** — 새 `ui/useDragScroll.ts` (마우스 전용): 4px 문턱
  전에는 클릭(날짜 정렬 토글)이 클릭으로 남고, 넘으면 그 제스처의 클릭
  하나를 삼킨다. pointerdown 에서 preventDefault — 텍스트 선택만 죽고
  click 은 기본 동작이 아니라 산다. 휠·스크롤바·터치는 원래대로. 컴파일러
  린트(react-hooks/refs) 함정 둘: 훅이 ref **객체**를 반환해도, 사용부가
  반환 객체를 점 접근해도 걸린다 — 콜백 ref(useMeasure 문법) + 구조분해
  (useFloatingWindow 문법)로 통과.
- **날짜·구분 열 스티키** (ForwardMatrix 의 scroll-affordance 전례) —
  팬 중에도 어느 날 어느 줄인지 안 잘린다. §G(sticky-opaque) 준수:
  bg-popover 동반. ⚠ `ch` 는 요소 자신의 font-size 로 풀린다 — 13px
  구분 헤더가 14px 표의 7ch 트랙에 붙으려면 `calc(7ch * 14 / 13)`,
  본문 구분 셀은 td 를 14px 로 두고 13px 는 안쪽 span 에.
- 검증: FE vitest 820/821(1 skip 기존) · lint 0 에러 · next build 통과 ·
  크롬 실화면(1Y 페이 100억 3/9 진입 백테스트 → 서랍 스택에서 CDP
  드래그로 scrollLeft 612 팬, 스티키 열·복원 테너 육안 확인). 백엔드
  무변경이라 pytest 는 안 돌렸다. ReconStack.test 의 숨김 핀은 복원
  핀으로 교체, useDragScroll.test 신설(팬 수식·문턱·클릭 삼킴·터치
  불간섭 4건).
