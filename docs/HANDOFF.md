...
**재투자 결함은 조달 쪽에서 고쳤다** [2026-08-14, 같은 날 뒤이어]. 처음에는
조달을 매수단가에 고정으로 붙였는데, 그러면 이표를 받아 쥐고 있는 동안에도 그
돈까지 빌리는 셈이다. 이제 **줄어드는 잔액**에 붙는다(`_net_funding`) — 이표와
만기 원금이 들어오면 그만큼 갚은 것으로 본다. 교과서의 자금조달 채권 수익률이
그 처리이고, **재투자 금리를 새로 정할 필요가 없다는 것**이 이 방식을 고른
이유다(갚는 금리 = 빌리는 금리).

실측(조달, 잔액기준 → 고정 대비): 3Y 2023-08 **+42,712,000**(앞서 추정한 재투자
43.2M 과 일치, 차이는 단리/복리) · 1Y 2025-08 +3,843,018 · 1Y 2021-03 +233,901.
만기가 휴일이라 종료일이 며칠 뒤인 경우의 초과 조달(실측 2일, 156만원)도 같은
기전으로 사라졌다.

**IRS 쪽은 못 고친다.** 조달 자체가 없어서(Cash Bond 전용 [OWNER]) 결제현금을
굴릴 금리가 없다. 그쪽 결제현금이 0% 로 앉아 있는 것은 남는다 — 고치려면 IRS 에
조달 개념을 들여야 하고, 그건 오너가 범위 밖으로 둔 것이다. [미해결]
