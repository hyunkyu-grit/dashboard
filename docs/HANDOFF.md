...
### 인트로가 커브 면이 됐다 — 능선 + 착지 (2026-08-14)

[OWNER, 2026-08-14 — "능선 + 착지"]. 인트로 커튼의 그림이 **부채에서 면으로**
바뀌었고, 걷힐 때 페이드 대신 **앱의 커브 자리로 날아가 앉는다**. 외부 리서치에서
가져온 형태는 NYT Upshot 의 3-D yield curve(2015, X=만기·Y=시간·Z=금리)와,
그것을 canvas 2D 로 그리는 1970년대 floating horizon 이다. 전체 설명은
DESIGN §14 「능선과 착지」.

- 바뀐 파일: `ui/introCurves.ts`(능선 투영·슬라이스·착지 타임라인, 전부 순수),
  `ui/IntroCurtain.tsx`(그리기·착지), `ui/App.tsx` + `ui/OverviewColumns.tsx`
  (착지 지점 표시), `guards/intro-curtain.test.ts`(38→54).
- **화면을 보고 배운 것 셋** — 셋 다 코드 주석에 실측으로 남겼다.
  1. 깊이 사다리를 크게(`RIDGE_RISE` 0.66) 주면 2016(1.2%)이 2026(4.1%) **위에**
     그려진다. 평면 스택으로 읽으면 그냥 거짓이다.
  2. 사다리를 죽이면(0.16) 레벨은 맞는데 입체가 사라진다 — 오너 판정이
     "입체라는 느낌이 전혀 안 들긴 해" 였다. 격자의 나머지 방향(리브)을 넣었더니
     **더 나빠졌다**: 열 위치가 레벨로 정해지니 리브가 위아래로 튀어 그물이 됐다.
  3. 진짜 원인은 기하가 아니라 **밀도**였다. 아홉 개는 어떻게 놓아도 선 아홉
     개다. `RIDGE_SLICES`(49)로 실측 앵커 사이를 보간해 채우자 가림이 지형이
     됐다. 앵커 아홉 줄은 더 짙게 남겨 어느 선이 측정된 날인지 구분된다.
- **착지 지점은 둘이고, 순서가 있다**: `data-intro-land="curve"`(우측 pane) →
  `"outrights"`(오버뷰 주요 아웃라이트). 두 번째가 핵심이다 — 앱은 `tab="all"`
  로 열리고 거기엔 우측 pane 이 아예 없어서, pane 만 알던 첫 판은 **시작
  경로에서 한 번도 착지하지 않았다**(늘 조용히 페이드).
- **「커튼은 반드시 나간다」는 안 건드렸다.** 착지는 걷힘의 장식이지 조건이
  아니다: 배경·로크업·캡션은 그대로 §14 EXIT 로 빠지고 그 뒤엔 앱이 다 보이며,
  날아가는 건 선 하나뿐이고, 언마운트 `setTimeout` 은 모든 분기 밖에서 `LAND_MS`
  냐 `FADE_MS` 냐만 고른다. 착지 지점을 못 찾는 것은 **정상 경로**다(실패 화면·
  좁은 창·프리뷰).
- 함정 둘, 둘 다 실측으로 잡았다. (1) 하이라이트를 면 위로 올릴 때 **채움까지
  올리면 페이지 색이 능선을 통째로 지운다** — 하이라이트는 선만 긋는다.
  (2) 사다리+레벨 합의 실제 범위는 데이터가 정하므로 remap 없이는 그림이 상자의
  가운데 띠(30~67%)에만 앉는다.
- 게이트: 919 통과 / 1 스킵, `pnpm build` 통과. 라이트·다크 양쪽 확인
  (가림이 페이지 색 채움이라 두 테마에서 배경과 정확히 맞아야 하고, 맞는다).
- 겹치는 레인 주의: 이 세션이 도는 동안 **다른 세션이 같은 리포에서 작업 중**
  이었다(Cash Bond·Setting·Lab). `pnpm build` 는 `.next` 를 쓰므로 그쪽
  `next dev` 와 부딪힌다 — 정적 검증용 `next start` 가 뜨지 않으면 그 이유다.

### Lab 을 비웠다 — 라고 할 때 살걸 내려감 (2026-08-14)

[OWNER, 2026-08-14 — "lab 탭에 있던 내용 삭제"]. 연구실의 첫 세입자
**라고 할 때 살걸**이 화면과 서빙 경로째로 내려갔다. 실험은 가장자리에서
들어와 피드백으로 졸업하거나 내려가고, 이것은 졸업하지 않았다.

- 지운 것: `backend/app/regret.py`, `events.replay_leading_events` +
  `REPLAY_LOOKBACK`(이 둘의 유일한 소비자였다), 요약 페이로드의 `regret` 키와
  그 캐시 계산(`main.py`·`build_static.py`), `lib/api.ts::RegretEntry`,
  `ui/RegretLab.tsx`, `tests/test_regret.py`, `guards/regret-list.test.ts`.
- 남긴 것: **탭 자체**. `ui/LabEmpty.tsx` 가 자리 표시로 서고 **서버를 부르지
  않는다**. `guards/regret-list.test.ts` 가 겸하던 "Lab 이 마지막" 핀은
  `guards/lab-tab.test.ts` 로 옮겼다 — 실험 하나가 내려갈 때 탭 순서 규칙까지
  같이 사라지면 안 된다.
- **SCHEMA_VERSION 은 그대로 7**. 캐시된 페이로드의 모양이 바뀐 게 아니라
  항목 하나가 없어진 것이라, 남은 v7 `regret` 캐시 파일은 이제 아무도 열지
  않는다. 이미 구워진 `frontend/public/api/wall/summary.json` 은 다음 굽기
  전까지 `regret` 키를 갖고 있고, 읽는 쪽이 없으므로 무해하다.
### Lab 의 두 번째 세입자 — 커브 표면 (2026-08-14)

[OWNER, 2026-08-14 — "Yield Surface … 시계열이랑 기간 구조 합친 거"]. 외부
리서치 → 선택지 → 구현까지 같은 세션에서 갔다. **판정 근거·대안 비교·실측은
전부 `docs/YIELD_SURFACE.md`** 에 있고, 여기에는 다음 사람이 걸릴 것만 적는다.

- 오너 선택 = **SVG 사투영**(의존성 0) · **라이브 노드 × 주별 × 전기간** ·
  **단면 2D + 역전 + 정책금리**.
- **노드는 9 다**(3M~10Y). "라이브 10노드" 의 10 에는 1D 가 있지만
  `CurveView::CURVE_NODES` [OWNER, 2026-08-05] 가 콜금리를 커브 점에서 뺐고,
  그 사유가 그대로 적용된다. 집합은 `QUOTED_NODES` 에서 **유도**한다.
- **가림이 이 그림의 진짜 위험이다.** 오늘 금리가 10년 레인지 꼭대기라
  최신 커브가 앞에 서면 벽이 된다 — 실측으로 RISE 0.55 에서 능선 67%가 완전히
  가렸다. **RISE 0.70** 으로 32.6%(=부분화소 적층)까지 내렸고, SPAN 0.30 이
  하한이다(더 내리면 커브가 납작해진다).
- **능선은 픽셀당 한 장이고, 사이는 보간한다** [OWNER — "자연스럽게 사이사이에
  보간하기"]. 첫 판(2.4px 피치)은 4주에 1장씩 버려서 **선 다발**로 읽혔다.
  개수는 데이터가 아니라 **상자 높이**가 정한다. 그리고 **밀도를 올리면 잉크를
  내려야 한다** — 0.55 로 픽셀당 한 장을 그리면 화면이 통째로 주황이 된다
  (지금 0.16 / 역전 0.55). 형태는 채움과 가림이 만들고 선은 결이다.
- **연도 라벨은 능선 뒤에 그린다.** 앞에 그리면 가까운 능선의 불투명 채움이
  전부 덮는다(실측으로 라벨이 통째로 사라졌다). 바닥선은 반대로 덮여야 맞다.
- **정책금리는 계단이 아니라 수준선**이다 — 능선 수백 개 위의 수직 사건선은
  잡음이 된다. `CurveView` 와 같은 판정.
- 인트로 커튼 레인이 같은 날 같은 기하를 깎았다. **방법(floating horizon ·
  밀도 · 리브 금지)은 가져왔고 사다리만 반대로 갔다** — 약속이 다르기
  때문이고, 이유는 YIELD_SURFACE.md §6.
- **정직한 한계**: 커브 한 장의 모양이 눌린다(레벨 3.9%p vs 커브 1.2%p).
  지금은 단면 패널이 모양을 진다. 표면이 모양을 말하게 하려면 z 를 디민해야
  하고, 그건 「금리 표면」이 아니라 「모양 표면」이라 오너 판정거리다.
- 새 엔드포인트 `/api/surface` 는 **정적 쌍둥이가 있다**. 새 고정 페이로드를
  더할 때 등록할 곳이 네 군데다: `static_paths.py` · `build_static.py` ·
  `guards/static-paths.test.ts`(세 목록) · `guards/live-routes-proxied.test.ts`.
  가드가 전부 잡아 준다 — 잡히면 그게 정상이다.
...
**2차 패스 — 아웃라이트와 같은 화면으로** [OWNER, 2026-08-14, 세 번의 되돌림
끝에]. 처음에 표+폼을 오른쪽 판에 접어 놓았고, 그 다음 떠 있는 창으로
갈아엎었다가, 오너가 "일관성 유지하라고 했지 언제 갈아엎으라고 했냐" 로
멈춰 세웠다. 최종형은 **셸의 두 판 문법 그대로**다:

- 왼쪽 표가 아웃라이트와 **같은 격자·같은 셀**이다. 52주 고·저·평균 + 위치
  슬라이더 + 세타를 `ui/RangeCells.tsx` 를 그대로 불러 그린다 — 그러려고 그
  파일의 프롭을 `Row` 에서 구조 타입 `RangeRow` 로 넓혔다(`PnlSeries` 와 같은
  수법). 헤더 정렬도 붙었다.
- 종목 열만 **200px** 로 넓혔다(`CASHBOND_LABEL_W`). `WIDEST.label` 은 라틴
  9글자(`1s1.5s10s`)에서 나온 값이라 "캐피탈채 AA- 10Y" 가 두 줄로 접혔다.
  그만큼 표 판도 880 → 980 이다. IRS 표는 안 건드렸다.
- 오른쪽은 **그 행의 시계열**(`PreviewChart`, 셸 미리보기 판과 같은 그룹박스).
- 백테스트는 창이고 제목은 그냥 **"백테스트"** 다.
- **자산스왑은 자기 탭**이다(현금채권 바로 밑). 한 표에 %와 bp 가 번갈아
  서면 어느 단위를 보는지가 사라진다.
- **방향 칸이 없다** [OWNER — "국고채는 매도는 없는거고"]. 백엔드도
  `run_backtest` 에서 `direction != 1` 을 거절한다 — 공매도는 대차료가
  필요한데 그 값이 없고, 0 으로 두면 공매도가 늘 이기는 백테스트가 된다.
- **세타는 순캐리**다 [OWNER 선택]: (쿠폰 − 조달) + 롤다운, DV01 백만원당,
  3개월, **매수 기준**(IRS 표는 페이 기준). 그래서 Setting 을 바꾸면 이 열이
  같이 움직이고, `/api/cashbond/instruments` 가 조달을 쿼리로 받는다.
  자산스왑 세타는 두 다리의 합이고 분모는 **채권 다리의 DV01** 이다(par-par
  라 순 DV01 이 0 에 가깝다 — IRS 가 스프레드에서 기준 다리를 쓰는 것과 같다).

**스크리너 칩은 은퇴했다** [OWNER, 2026-08-14 — "이제 스왑에서도 없애기"].
IRS 표에서도 뺐다: 정렬과 52주·위치·세타 열이 같은 질문에 이미 답한다.
`ui/screener.ts` 의 프리셋 정의는 연구실이 아직 써서 남겼다.

### 만기 보유가 원금만큼 손실로 찍히던 것 (2026-08-14)

[OWNER — "만기까지 보유한 채권 백테스트 손익이 이상한 이유 찾았어"]. 맞다. 결함
둘이 겹쳐 있었고 **총액과 분해에서 다르게** 드러났다.

**① 결제현금에 액면 상환이 없었다.** `dirty` 는 아직 안 온 현금흐름의 현재가치라
만기에 0 이 된다. 그때 나간 액면이 현금에 안 잡히면 `dirty + 현금` 이 1.04 에서
0.04 로 **떨어진다**(1Y·쿠폰 4% 실측). 손익 = Δdirty + 결제현금 이라는 항등식
자체는 옳고 **결제현금의 정의가 좁았다**. FTSE Russell 의 Guide to Calculation 이
total return 의 cash 항을 "coupons **and any principal repayments** … cumulative
cash from principal redemption" 으로 정의하는 그대로다.

**② 고치니 그 액면이 캐리로 들어갔다.** 만기 보유 3Y 의 캐리가 110억, 롤다운이
−102억으로 찍혔다. 상환된 액면은 **소득이 아니라 자본 회수**라 가격 쪽이다:
`clean = dirty − 경과이자 + 상환액면`. par 로 사서 par 를 돌려받았으므로 만기에
평가 + 롤다운이 **정확히 상쇄**되어야 하고, 지금 그렇게 된다.

고친 뒤 실측(합성 커브 아님, 실제 민평):

    국고 3Y 2023-08-01 → 2026-07-31 (만기)
      평가 +223,374,748 · 롤다운 −223,374,748  ← 정확히 상쇄
      캐리 1,095,000,000 = 100억 × 3.65% × 3년  ← 정확
      조달 −929,178,082
      총액 +165,821,918 = 쿠폰 − 조달  ← 정확

**스왑에는 이 결함이 없다.** 바닐라 IRS 는 원금 교환이 없어 잃을 원금이 애초에
없고, 만기까지 `dirty + 현금` 이 매끄럽다(1Y 페이 100억으로 실측). 만기에 clean·
경과이자가 0 이 되고 전액이 결제현금에 남는다.

**가격식 자체는 검증됐다.** 표준 폐형식 `P = (c/y)(1−(1+y)^−n) + (1+y)^−n` 과
쿠폰·수익률·만기 조합에서 **1e-15 까지 일치**한다(할인채 포함).

**조달은 초기 투자금액에 붙는다** [OWNER, 2026-08-14 — "조달은 초기 투자 금액
기준으로 붙여야 함"]. par 로 샀으므로 곧 액면이고, 이표가 들어와도 줄지 않는다.

같은 날 잠깐 **잔액 기준**으로 바꿨다가 오너가 되돌렸다. 들어온 이표가 차입을
갚아 나가는 처리였고 크기도 쟀다(3Y 만기 보유 +42,712,000 = 손익의 3.9%, 만기가
휴일일 때의 초과 조달 156만원도 같이 사라졌다). 되돌린 근거는 데스크가 조달을
잡는 방식이다 — 텀 레포로 매수금액을 통째로 조달하고, 이표로 그때그때 차입을
갚는 것은 별개의 자금관리이지 이 포지션의 비용이 아니다. 구현과 실측은
`git show 021f2894` 에 남아 있다.

**받은 현금은 굴리지 않는다 — 규약이지 결함이 아니다** [OWNER, 2026-08-14 —
"이건 굴리는거 상정 안 하긴 해"]. 채권도 IRS 도 결제현금을 0% 로 들고 있고,
그것이 의도한 바다. 한때 [미해결]로 적어 두었으나 오너가 정리했으므로 내린다.

조달을 초기 투자금액에 붙이는 것과도 앞뒤가 맞는다: 매수금액을 조달해서 사고,
들어온 이표는 굴리지 않고 그대로 쌓는다. 크기만 남겨 둔다 — 3Y 쿠폰 10.95억을
2.8% 로 굴렸다면 43M(손익의 3.9%)이고, 이 화면의 손익은 그만큼 보수적이다.

**오너 예시로 검증**(쿠폰 2.5% · 할인율 2.5% · 100억 · 분기 · 2년): 이표 8회 ×
62,500,000 = 5억, 마지막 이표와 함께 액면 100억 수령, 잔여 평가 0. 평평한 커브라
평가·롤다운 모두 정확히 0 이고 캐리가 5억, 조달 −626,232,877, 합계 −126,232,877
로 다섯 칸이 닫힌다.

### 세타를 하루치로 — 그리고 하루로 재면 안 되는 이유 (2026-08-14)

[OWNER — "세타 전부 다 하루치로 바꾸고, 외부 리서치를 통해서 실제와 정합한지
확인하고, 채권에서는 조달 차감하지 않는 걸로"]. 셋 다 했고, 가운데 것이 앞의
것을 고쳤다.

**호라이즌을 문자 그대로 하루로 두면 롤다운이 잡음이 된다.** 파 금리를 T 와
T−1일에서 각각 스왑으로 구성하면 스케줄 이산화와 노드 사이 보간이 진짜 롤보다
큰 점프를 만든다 (실측 asof 08-13):
...
그래서 **계산 창은 분기로 두고 표기만 일 단위로 나눈다**(`theta.HORIZON_DAYS`,
나누는 자리는 `_block` 과 `theta_for_bond` 두 곳뿐). 캐리는 시간에 선형이라
나누기가 정확하고, 롤은 "앞으로 한 분기의 하루 평균" 이 된다 — 데스크가 bp/일
을 말할 때 뜻하는 것도 그것이다. 진짜 하루 롤이 필요하면 커브에서 dPar/dT 를
해석적으로 뽑아야 한다. [미해결]

검산(둘 다 실측으로 일치): IRS 3Y 페이 하루 캐리 −249,782 ≈ 100억 ×
(CD 2.93 − 3.84)% ÷ 365 = −249,315. 국고 3Y 하루 쿠폰 1,034,795 = 100억 ×
3.777% ÷ 365 **정확히**. IRS 3Y 하루 롤이 −992,151(순진한 1일) → −58,950(분기
평균)으로 제자리를 찾았다.

**외부 확인이 오너 결정 하나와 어긋난다 — 그대로 적어 둔다.** 채권 캐리의
시장 표준은 `carry = y − r_f`, 즉 **조달을 뺀다**: yieldcurve.pro 는 "You
collect coupon, you pay financing, and the difference is yours", AnalystPrep 은
"income a bond earns above its funding cost". 오너는 IRS 세타와 정의를 맞추려고
빼지 않기로 했고(2026-08-14), 그 대가는 이 열이 **총 쿠폰**이 되어 수익률 높은
종목이 늘 위에 선다는 것이다(캐피탈채 > 국고채, 늘). 되돌리려면
`cashbond.theta_for_bond` 에 조달 한 줄을 다시 빼면 된다 — 조달값 자체는
Setting 이 여전히 들고 있고 백테스트의 조달 칸이 쓴다.

DV01 정규화는 관행 범위 안이다: Deriscope 는 "either spot or the forward (as of
T) flat DV01" 이라 적고, 이 리포는 스팟을 쓴다(블룸버그는 포워드).

정적 굽기 산출물도 다시 구웠다 — `summary.json` 이 `horizonMonths` 를 들고
있어서 배포본의 타입이 깨졌을 것이다. 바뀐 파일은 둘(summary + manifest)뿐이다.
...
**화면은 그 칸을 안 그린다** [OWNER, 2026-08-14, 같은 날 뒤늦게 — "개시손익
적으면 걍 무시해도 될 거 같은데"]. 표에서는 평가에 접힌다: 절대 금액이 늘 작고
(1년 보유 IRS 10Y 에서 −5만원, 총손익의 0.005%) 열 하나를 줄 값이 아니라는
판단이다. **엔진은 그대로다** — 그 밤을 따로 세어 롤다운에서 빼 두는 것이
이 수정의 요점이었고, 접는 것은 `ui/krw.ts:splitKrw` 한 곳의 표시 결정이다.
다시 펴려면 거기만 고치면 된다.
