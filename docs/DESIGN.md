...
`_is_kr_business_day`; `lib/freshness.ts` counts how many have passed against
the reader's local date. The calendar therefore is not duplicated in
TypeScript — only the answer is shipped. Better than before, too: the age now
follows the *reader's* clock rather than a server's.
