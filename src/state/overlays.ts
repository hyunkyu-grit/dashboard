...
/**
 * 기본값 — **MA20 하나만 켠다** [OWNER, 2026-08-26 — "MA120도 끌 수 있게 …
 * 기본값으로 꺼져 있으면 좋겠다"].
 *
 * 처음엔 20·120 둘이었다. 120=반기 선은 이 차트에서 거의 평평해서 «켜져 있는
 * 줄 몰랐다» 가 되기 쉽고, 그 상태로 잉크만 한 겹 더한다. 20=1개월이 이 데스크가
 * 실제로 읽는 하나이고, 그것도 위에서 «겹치는 뜻이 없는» 초록을 받는다.
 *
 * 나머지 넷은 **켜는 순간 읽는 사람이 고른 것**이다 — 색 배정은 그대로 두므로
 * 켜면 바로 제 색으로 선다.
 */
