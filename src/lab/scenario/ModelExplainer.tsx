...
/* ⚠ **물가 손잡이의 필립스 배선이 논문의 식과 다르다** [실측 2026-08-21].
 *
 * BOK WP 2025-3 을 열어 식 (23)·(24) 를 **눈으로 읽었다**(PDF 텍스트 추출은
 * 기호를 날려서 BIGFOOT 이 표의 값만 보고 배치를 추정했다 —
 * `PROVISIONAL_PHILLIPS`. 그런데 **배치는 추정할 필요가 없다. 논문 25쪽에
 * 인쇄돼 있다**).
 *
 *   논문 eq (23)  π_Core,t = (1−φ1−φ2)·π̄_t + φ1·π_Core,t−1
 *                            + φ2·E_t π_Core,t+1 + φ3·ŷ_t
 *   Table 8       φ1 = 0.25 · φ2 = 0.15 · φ3 = 0.10 · π* = 0.02
 *                 δ1 = 0.7232 · δ2 = 0.3164            (기호까지 확인)
 *
 *   항                     논문        BIGFOOT 배포 (`phase3.FINAL_OPTIONS`)
 *   어트랙터 π̄            0.60        0.15
 *   직전 코어 π_{t−1}      0.25        0.10
 *   기대                   0.15 (1분기) 0.50 (위성 VAR, **4분기**)
 *   GDP 갭                 0.10        0.25
 *
 * 배포판은 `phillips_perm = {lag 0.10, att 0.15, gap 0.25}` — Table 8 값들의
 * **순열 탐색 결과**지 논문의 배치가 아니다. 갭 기울기가 2.5배 가파르고
 * 어트랙터 무게가 1/4 다.
 *
 *   논문 eq (24)  π̄_t = (1−δ1)·π* ÷ 4 + δ1·π̄_{t−1} + δ2·(π_Core,t−1 − π̄_{t−1})
 *                      = (1−δ1)·π* ÷ 4 + 0.4068·π̄_{t−1} + 0.3164·π_Core,t−1
 *   배포 «nested»       0.7232·π̄_{t−1} + 0.0876·π_Core,t−1 + 0.1892·π*
 *
 * 배포판의 어트랙터는 훨씬 더 관성적이고(0.72 vs 0.41) 실현 코어에 3.6배 덜
 * 반응한다. 논문이 어트랙터를 둔 이유가 «(de)anchoring 을 말할 수 있게» 인데,
 * 그 채널이 약해져 있다.
 *
 * **고치려면 `basis.json` 을 다시 구워야 한다** — BIGFOOT 의 방정식을 고치고
 * 전 기저를 재생성하는 일이라 이 리포 밖이다. 여기서는 사실만 적고, 아래 표의
 * «물가» 줄이 화면에서 그 사실을 말한다. */
const DRIVERS: { key: string; label: string; note: string; bases: string[] }[] = [
  {
    key: 'cpi',
    label: '물가',
    note: '필립스 eq.23·24 — 배포 계수가 논문의 배치와 달라요',
    bases: ['cpi'],
  },
