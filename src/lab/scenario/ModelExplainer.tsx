...
/* 이 탭의 다섯 손잡이는 **논문에 인쇄된 식 그대로** 돈다 [2026-08-21].
 *
 * BOK WP 2025-3 의 44개 식을 전부 열어(텍스트 추출은 수식 기호를 날리므로
 * 페이지를 이미지로 렌더해서) 배포 배선과 한 줄씩 대조했고, 어긋난 자리를
 * 다 맞췄다. 값 옮겨 적기는 처음부터 맞았고 틀린 것은 늘 **배치**였다.
 *
 *   eq (23)(24) 필립스   어트랙터 0.15→0.60 · 갭 0.25→0.10 · 기대 4분기→t+1
 *                        (배포는 Table 8 값의 순열 탐색 결과였다)
 *   eq (8)(11)  소비·설비 갭 항이 **수준**이었는데 인쇄된 것은 **Δŷ**(변화)
 *   eq (19)     수출     환율이 Δ 였는데 인쇄된 것은 **Δ₄**(전년동기)
 *   eq (40)-(43) 대출금리 «구성된 해석»(전가 0.54)이었는데 인쇄된 것은
 *                        전가 1.0 + 스프레드에만 AR (Table 16 이 확인)
 *   eq (12)-(16) 건설·정부 **외생으로 얼어 있었다** — 살렸다. 건설은 GDP 의
 *                        14.3%·수입수요의 22% 인데 둘 다 0 이었다.
 *
 * ── 무엇이 좋아졌나 ────────────────────────────────────────────────────────
 * 국내 기저가 훨씬 잘 잦아든다. 예전엔 `gap`·`exports` 기저의 **최댓값이
 * q24 에 있었다**(|q24|/max = 1.00 · 0.90 — 6년 지평 끝까지 자라고 있었다).
 * 지금은 꺾여 내려온다(0.21 · 0.17). 정상상태 잔차도 전부 0 이 됐다.
 *
 * ── 대가와 남은 것 ────────────────────────────────────────────────────────
 * 논문 IRF 밴드 대조는 12/13 → 9/13 이다. 다만 그 12/13 은 **그 밴드에 맞춰
 * 필립스 순열을 고른 결과**라 같은 잣대가 아니고, 면책조항까지 썼던 유일한
 * 실패(A 충격 CPI 저점)는 인쇄된 형태에서 밴드 안으로 들어왔다.
 *
 * 남은 실패 넷은 전부 **금리→주택→부채→소비** 사슬의 진폭이고, 뿌리는 논문이
 * 안 박은 «금리 단위» 규약이다. 주택식 (27)(28)이 함의하는 장기탄력성이
 * %p 당 −41.5 로 비현실적이라 원저자는 소수 단위로 추정한 것으로 보이는데,
 * 논문에 단위가 안 적혀 있다. 밴드를 맞추려 배율을 지어내지 않았다 — 그게
 * 애초에 이 라운드가 고치러 온 잘못이다.
 *
 * 데이터로 못 메우는 것: eq (11) 의 DRAM 은 **Gartner 반도체 초과수요 지수**
 * (유료), eq (29)(30) 의 세계 수출물가, eq (12)(14) 의 건축착공은 ECOS 가
 * 2013년부터만 싣는다. 셋 다 외생이라 기저는 안 바뀌고 과거 잔차만 깎인다. */
