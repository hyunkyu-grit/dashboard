  ...
  return (
    <HStack gap={2} width="100%" alignItems="stretch" flexGrow={1} minHeight={0}>
      ...
      >
        ...
        >
          {/* Fed 손잡이는 내려 두었다 (`scenario/combine.ts` 의
              `US_BASES_USABLE`).

              **여기 적혀 있던 이유가 틀렸다** [2026-08-21]. «400배 어긋난다» 는
              첫 진단은 그날 안에 철회됐는데(지평을 잘못 맞춘 비교였다 — 맞추면
              GDP 갭이 −0.5093 vs −0.5111 로 소수 셋째 자리까지 같다) 코드 주석만
              고치고 **화면 문장은 안 고쳤다.** 트레이더에게 틀린 이유를 말하고
              있었다.

              진짜 이유는 발산이다. `combine.ts` 의 표 참조. */}
          <Text as="p" font="legal" color="fgMuted">
            Fed 손잡이는 지금 내려 뒀어요. 미국 기저가 24분기에 걸쳐 잦아들지 않고
            커지는 데다, 충격을 몇 분기 끄느냐만 바꿔도 6년 뒤 금리의 부호가
            뒤집혀요(2분기 +76bp · 6분기 −374bp). 국내 기저는 정상적으로 잦아드는
            걸로 봐서 경제 반응이 아니라 조립 문제예요. 고쳐서 다시 구운 뒤에
            올릴게요.
          </Text>
          <HStack gap={1.5} alignItems="flex-end" flexWrap="wrap">
            <Box width={104}>
