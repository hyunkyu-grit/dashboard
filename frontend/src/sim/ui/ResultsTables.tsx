 ...
 * `portfolio_krd_day`). 행 하나가 세 지표를 다 들고 있다: `pvbp`(그날
 * KRD), `dailyDbp`(그날 테너별 금리 변화), `pnl`(둘의 곱 — 선형 추정 손익).
 * 여기서 계산하는 것은 없다 — 행을 세 줄로 펴서 그릴 뿐이다. (한 행 한
 * 줄에 15개 테너 × 3값을 다 실으면 아무것도 못 읽는다는 판단은 그대로다 —
 * 답이 렌즈 토글에서 세로 스택으로 바뀌었을 뿐이다.)
 *
 ...
 * 백엔드를 정렬했다, chart.py `_pvbp_prev_r`). 그래서 KRD 줄(그날의 KRD
 * 수준값)과 Δbp 줄을 곱해도 손익 줄과 정확히 일치하지 않는다 — 특히
 * 이벤트·리픽싱으로 KRD 가 점프한 날. 세타(캐리+롤다운)의 정확한 정의:
 ...
 * 중 대여섯만 산다. 날짜는 MM-DD(연도·D+n 은 title)로 줄인다. 그래도 전
 * 테너가 살아 있는 북은 가로 스크롤이 남는다 — 그건 데이터의 폭이지 화면의
 * 낭비가 아니다. (이 규율들의 구현은 이제 ui/ReconStack.tsx 에 산다.)
 *
  ...
  return (
    <div>
      ...
      <p className="pb-2 text-callout text-ink-2">
        하루가 세 줄이에요 — KRD(그날 커브 감도), Δbp(그날 변화), 손익(전일 KRD ×
