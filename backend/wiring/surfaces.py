...
EQUATIONS = [
    ...
    ("9", "설비투자 목표", "expenditure",
     ...
     "**`− UC_I` 가 안 배선돼 있어요.** 목표 편차를 문자 그대로 0 으로 둬서, "
     "설비투자로 들어오는 화살표가 산출갭 하나뿐이에요."),
    ("10", "설비투자 자본 사용자비용", "expenditure",
     "UC_I = (기업대출금리 + 회사채금리)/2 − CPI상승률/4 + 감가상각률",
     "안 배선돼 있어요 — eq (9) 가 이걸 안 받으니 계산할 자리가 없어요. "
     "건설투자(eq 13)는 같은 것을 제대로 씁니다."),
    ("11", "설비투자 PAC", "expenditure",
     "오차수정 + AR + **기대 할인합** + γ_I1 Δŷ + γ_I2 Δln P_I + γ_I3 ln DRAM",
     "배선된 것은 오차수정·AR·Δŷ 셋뿐이에요. 기대항 없음, 투자디플레이터 "
     "없음, DRAM 지수는 Gartner 유료라 없음. **코드가 이 식을 eq 10 이라고 "
     "부르는데 인쇄 번호는 11 이에요**(PDF 인쇄 p.18)."),
...
LEDGER = [
    ...
    {
        "key": "fi-usercost",
        "title": "설비투자에 금리가 안 들어가요 — eq (9) 의 `− UC_I` 미배선",
        "paper_says": "eq (9) 는 설비투자 목표에 자본 사용자비용을 **계수 −1 로** "
                      "넣고, eq (10) 이 그 사용자비용을 기업대출금리·회사채금리·"
                      "물가·감가상각으로 정의해요.",
        "we_do": "목표에 상수·잠재산출·코로나더미만 넣고, 솔버는 목표 편차를 "
                 "**문자 그대로 0** 으로 둬요.",
        "why": "이관 시점에 그렇게 배선돼 있었어요. 2026-08-21 배선 대조에서 "
               "찾았어요.",
        "could_be_wrong": "**정책금리가 설비투자에 닿는 통로가 산출갭 하나뿐**"
                          "이에요. 건설투자는 같은 사용자비용이 제대로 배선돼 "
                          "있어서 두 블록이 비대칭이고, 논문의 미국 +25bp 패널에 "
                          "있는 설비투자 −0.02% 를 우리 방식으로는 재현할 근거가 "
                          "약해요.",
        "paper": "eq (9)(10), 인쇄 p.18",
        "code": "backend/bigfoot/equations/korea.py::investment_fi_target · "
                "backend/bigfoot/solve/system.py (`0.0 - lag(\"i_fi\", t)`)",
        "node": "i_fi",
        "equation": "9",
    },
    {
