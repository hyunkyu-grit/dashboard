...
#: 코드가 스스로 적은 `eq_no` 와 인쇄 번호가 갈리는 자리. **지금은 비어 있다.**
#:
#: 여기 한 줄이 있었다 — `investment_growth` 가 스스로를 eq 10 이라 불렀는데
#: PDF 인쇄 p.18 에서 (10) 은 자본 사용자비용 정의이고 설비투자 PAC 성장식은
#: (11) 이다. 엔진이 동결이라 이 표가 우회하고 있었고, 2026-08-21 (P4) 에
#: `korea.py` 를 직접 고쳐서 표를 은퇴시켰다.
#:
#: **비워 두고 없애지 않는다.** 같은 일이 다시 생기면 여기에 서야 하고,
#: 화면은 「지금은 갈리는 자리가 없어요」 를 말할 수 있어야 한다.
EQ_NO_CORRECTIONS: dict[str, dict] = {}
