...
def spread_vol_hist_series(seq: list[float | None]) -> list[float | None]:
    ...
    **오히려 조금 더 붙어 있다**. 셋 중 무엇을 넣든 상관이 ~+0.79 인 것은 Score
    ...
    ## 남은 +0.60 은 어디에 사는가 [2026-08-21 실측 — 위 문단의 오독을 막는다]

    남은 상관을 "캐리가 신용위험을 따라가서"로 읽으면 **틀린 자리를 고치게
    된다**. 캐리는 애초에 점수에 안 들어간다. 50:50 을 갈라 재면(H=6M, n=42):

        corr(Score,        스프레드) = +0.610
          ├ 절대축 spreadVolPct     = +0.358   ← 앵커 교체가 고친 쪽
          └ 상대축 relRv 랭크       = +0.674   ← 남은 상관은 여기 산다

    즉 새는 곳은 **deviation 만 쓰기로 한 상대축**이다. 그리고 그것은 오늘의
    우연이 아니다 — 42계열 횡단면으로 corr(스프레드 수준, 자기이력 z)를 최근
    252영업일 재면 중앙값 **+0.59**, 음수인 날은 **7%** 뿐이다.

    기전: z 는 σ 로 나누지만 넓어지는 국면에서 넓은 섹터는 자기 σ **보다 더**
    넓어진다(베타 > 1). 그래서 z 를 씌워도 수준이 다 안 지워진다. 섹터 평균
    수준에서도 Spearman +0.750 이고 가장 좁은 둘이 바닥이다(통안 −2.1bp →
    Score 38.6 · 은행 6.3bp → 51.0).

    **그래서 이 잔여는 버그가 아니라 크레딧을 순위 매기는 일의 값이다.**
    고치려면 relRv 의 z 정의를 건드려야 하고 그건 별개의 레인이다 — 여기서
    캐리 축이나 앵커를 다시 만지는 것은 헛다리다."""
    step = 63
    win = ANNUAL_OBS
    n = len(seq)
