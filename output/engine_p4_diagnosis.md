...
진단이 세운 일곱 단계가 다 끝났다. 순서는 진단이 적은 대로 갔다.

| 단계 | 무엇 | 결과 |
|---|---|---|
| D.1 | eq (9) `− UC_I` 배선 | 완료 (`18e9d1d`) — 설비투자 2.1배, **실패 넷은 안 움직임** |
| D.2 | γ_I2 는 편차에서 구조적 0 | 완료 — 코드가 말한다 (`korea.py`) |
| D.3 | `eq_no` `'10'`→`'11'` | 완료 (`d2f0e1b`) — 엣지 재키잉 0건 |
| D.4 | ρ = 0.801 잔차 감쇠 | **아래** |
| D.5 | `EcosDataError` + `blocked` 승격 | 아래 |
| D.6 | `solve/config.py` 상수 분리 | 아래 |
| D.7 | 산출물·픽스처·가드 재생성 | 아래 |

## D.4 — 못 창이 끝난 뒤의 준칙 잔차

설계는 §C.6 그대로다. 새 인자 둘(`pin_window` · `tail_rho`)이 생겼고, 창을
안 주는 호출자에게는 **이 기능이 존재하지 않는다**(IRF·조건부 예측·커브 조립이
전부 그쪽이다). `tail_rho=0` 이 급단절을 **바이트 단위로** 복원한다.

### 예측한 대로 된 것

- **기저 열다섯 중 `policy_q8` 하나만 움직였다.** 나머지 열넷의 12개월 IRS
  델타 변화가 다섯 테너 전부 `+0.000`.
- 선형성 게이트가 노이즈에 그대로 앉았다 — a: 0.0002bp, b: 0.0001bp.
- **스코어카드 9/13 불변.** IRF 는 못을 안 쓰니 앵커 13칸이 전부 `+0.00000`.

### 예측 안 한 것 — 화면의 숫자가 절반이 됐다

```
지속 −25bp × 8분기, 9~12분기 되돌림
    급단절  [ 8.457, 15.926, 22.392, 27.835]  →  지평 이탈 +27.84bp
    감쇠    [ 2.117,  5.644, 10.018, 14.791]  →  지평 이탈 +14.79bp

계단 −25/−50×7
    급단절  +54.14bp  →  감쇠 +28.96bp

12개월 IRS 델타 (지속 −25×8)
    2Y   +8.159 → +3.503        3Y  +11.686 → +8.107
    5Y   +7.542 → +8.120       10Y   +2.554 → +2.582
```

이유는 한 문장이다. **완화 쪽으로 벗어난 잔차가 경로가 끝나는 순간 증발하지
않는다.** 예전 숫자가 컸던 것은 「25bp 인하를 세 분기 만에 전부 되돌리고
넘어선다」 였는데, 그 급단절은 정한 것이 아니라 안 채운 것이었다.

3Y 의 준칙 몫이 33% → 4.5% 로 내려갔다. 「경로 그대로」 칸(+8.33bp)은 안
움직인다 — 못 창 **안**은 그대로니까.

### 새로 생긴 한계 — 24분기가 `policy_q8` 을 다 못 담는다

기저 꼬리 `|q24|/max` 가 `policy_q8` 에서 크게 올랐다:

```
y_gap 0.054→0.473 · cpi_yoy 0.402→0.788 · hpi 0.072→0.522
debt  0.374→0.762 · i_kr    0.248→0.427
```

꼬리가 24분기 안에서 안 잦아든다. `_irs_diff_path` 가 T 밖을 `PHI_I_TAIL=0.85`
로 외삽하므로 그 외삽이 지는 무게가 늘었다. 실제 영향은 작다(10년 12개월
델타가 0.086bp 움직였다) — `0.85^20 ≈ 0.039` 라 죽는다. **그래도 이건 기록해
둘 한계다.** T 를 늘리는 것은 별도 판단이고 여기서 안 했다.

### 화면이 지던 이름이 거짓이 됐다 — 같이 고쳤다

`strategy/risk.ts::RESIDUAL_TREATMENT` 가 `'급단절'` **상수**였다. D.4 뒤에는
화면이 「급단절」이라고 말하면서 감쇠된 숫자를 보여 주는 상태가 된다. 이름을
기저에서 읽게 바꿨다(`scenario_basis.json::residual_tail.treatment`) — 두 벌이면
한쪽만 낡는다는 이 리포의 상습 병이다.

## D.5 — 데이터가 없으면 죽는 대신 던진다

`EcosDataError(RuntimeError)` · `FredDataError(RuntimeError)` 를 세우고 `sys.exit`
넷을 `raise` 로 바꿨다. 진단이 짚은 것보다 한 자리 더 고쳤다.

- **키가 없는 것을 폴백에 태웠다.** `fetch_ecos` 는 `_api_key()` 를 `try` 밖에서
  불렀다 — 캐시가 멀쩡히 있어도 키가 없으면 굽기가 멈췄다. 안으로 넣었다.
  FRED 는 `except` 에 `RuntimeError` 를 더했다.
- **FRED 의 경고 줄이 리베이크에 안 잡히고 있었다.** `[warn] FRED fetch failed
  for X` 는 스캐너가 찾는 `[warn] fetch failed` 를 **안 담는다.**
  `[warn] fetch failed for fred:X` 로 바꿔서 이제 `blocked_on` 에 이름이 선다.
- **굽기가 멈추면 `engine_status.json` 만 앞으로 간다**(`_blocked_status`).
  원자성만 있으면 어제 판이 그대로 남고 그 판의 `staleness.state` 는 `fresh`
  라서, 화면이 「막혔다」 대신 어제의 「신선하다」 를 말했다.

이 자리가 실제로 살아 있었다는 증거가 이 세션에 있다. 이 PC 에는
`FRED_API_KEY` 가 없어서 `python -m wiring.surfaces` 가 D.5 **전이었다면 그
자리에서 죽었다.** 지금은 경고 일곱 줄을 찍고 캐시로 돈다.

## D.6 — 굽는 경로에서 plotly 를 끊었다

상수 여섯을 `bigfoot/solve/config.py`(import 없음)로 내리고 `phase3.py` 가
재수출한다. `conditional/hfl.py` 의 모듈 스코프 plotly 는 `render()` 안으로
넣었다. 실측:

```
bigfoot.scenario_basis.build · irs_curve.assembler · conditional.hfl
bigfoot.solve.system · wiring.edges · wiring.surfaces · rebake.__main__
    → 일곱 다 'plotly' not in sys.modules
```

`tests/engine/test_runtime_imports.py` 가 하위 프로세스로 다시 잰다(같은
프로세스면 다른 테스트가 이미 올려 놨을 수 있어서 못 믿는다). `config.py` 의
「import 없음」 계약도 AST 로 붙든다.

## D.7 — 재생성과 핀

리베이크 · `wiring.surfaces` · `parity_vectors` 를 다 돌렸다. 값 핀 둘이
깨졌고(예측한 자리다) 새 값으로 갈았다:

```
model-strategy-risk       되돌림 27.79 → 14.79bp
model-strategy-decompose  3Y 총 11.686 → 8.107 · 준칙 3.863 → 0.366 · 몫 33% → 4.5%
```

`ledger` 에 `residual-tail` 행을 세웠다 — ρ 가 우리 값이라는 것, 잔차 평균이
0 이 아니라 준칙 미스스펙을 같이 담을 수 있다는 것(추세 제거 시 0.78), ρ=0 이
복원이라는 것을 「어떻게 틀릴 수 있나」 칸에 적었다.

게이트: 엔진 pytest **60 통과 / 1 실패**(`test_ou_fit_sane_and_json_locked` —
기존 OU 핀, 이 레인과 무관) · 리베이크·ECOS·랩 52 통과 · vitest **1251/1251** ·
